...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rp[i][j][0] = RangeProof(32);
            rp[i][j][0].range[0] <== 0;
            rp[i][j][0].range[1] <== 9;
            rp[i][j][0].in <== puzzle[i][j];         //assert(puzzle[i][j]>=0); //[assignment] change assert() to use your created RangeProof instead
            rp[i][j][0].out === 1;                   //[assignment] change assert() to use your created RangeProof instead
            rp[i][j][1] = RangeProof(32);
            rp[i][j][1].range[0] <== 0;
            rp[i][j][1].range[1] <== 9;
            rp[i][j][1].in <== solution[i][j];      //[assignment] change assert() to use your created RangeProof instead
            rp[i][j][1].out === 1;                  //[assignment] change assert() to use your created RangeProof instead
